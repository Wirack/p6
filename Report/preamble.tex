\documentclass[10pt,a4paper,twoside]{memoir}
\usepackage[utf8x]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[table]{xcolor}
\usepackage[english]{babel}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{commath}
\usepackage{graphicx}
\graphicspath{{media/}}					% Directory that holds images.
\usepackage{lastpage}
\usepackage{afterpage}
\usepackage{lipsum}						% Lorem Ipsum
\usepackage{enumitem}
\usepackage{listings}
\usepackage{lstlinebgrd}
\usepackage[final]{fixme}
\usepackage{url}
\usepackage{epstopdf}

\usepackage{refcount}
\usepackage{caption}
\usepackage{subcaption}
\usepackage[small,compact]{titlesec}
\usepackage[bookmarks=true,		
			bookmarksopen=true,
			bookmarksnumbered=true,
			colorlinks=true,
			breaklinks=true,
			linkcolor=black,
			citecolor=black,
			urlcolor=black]{hyperref}	% Cross reference, clickable links and refs.

\usepackage[numbers,sort&compress]{natbib}			% BIB Citation
\usepackage{tikz} 
\usetikzlibrary{shapes,arrows}
\usetikzlibrary{arrows}
\tikzstyle{linear} = [draw, -latex']
\tikzstyle{firkant} = [ font={\bfseries},outer sep=4,inner sep=7,minimum size=0.5cm,line width=1, text=black, very thick, draw=black!55, top color=white, align=center]
\tikzstyle{rude} = [ draw, diamond, aspect=2 ,font={\bfseries},outer sep=4,inner sep=7,minimum size=0.5cm,line width=1, text=black, very thick, draw=black!55, top color=white, align=center]
\tikzstyle{io} = [font={\bfseries},trapezium,trapezium left angle=70,trapezium right angle=-70,minimum height=0.5cm,line width=1, text=black, very thick, draw=black!55, top color=white, align=center ]

\usepackage{footnote}
\makesavenoteenv{tikzpicture}
\usepackage{tabularx}
\newcolumntype{R}{>{\raggedright\arraybackslash}X}

\bibliographystyle{unsrtnat} %Take care of bib

\definecolor{bluekeywords}{rgb}{0.13,0.13,1}
\definecolor{greencomments}{rgb}{0,0.5,0}
\definecolor{green}{rgb}{0,0.9,0}
\definecolor{redstrings}{rgb}{0.9,0,0}
\definecolor{lgray}{rgb}{0.86,0.86,0.86}
\definecolor{baige}{rgb}{0.92,0.92,0.92}
\definecolor{maroon}{rgb}{0.5,0.0,0.0}
\lstdefinestyle{specialJava}{language=Java,
  showspaces=false,
  showtabs=false,
  breaklines=true,
  showstringspaces=false,
  breakatwhitespace=true,
  escapeinside={(*@}{@*)},
  commentstyle=\color{greencomments},
  keywordstyle=\color{bluekeywords},
  stringstyle=\color{redstrings},
  basicstyle=\ttfamily\small,
  frame=tb,
  numbers=left, 
  stepnumber=1, 
  numberstyle=\tiny, 
  numbersep=10pt,
  captionpos = b,
  linebackgroundcolor={\ifodd\value{lstnumber}\color{lgray}\else\color{baige}\fi}
}
\lstset{style = specialJava}

\usepackage{refcount}

\newcommand{\pagedifference}[2]{%
  \number\numexpr\getpagerefnumber{#2}-\getpagerefnumber{#1}\relax} %To calculate the amount of pages between two pages

\renewcommand{\figref}[1]{\hyperref[#1]{Figure \ref{#1}}} %Hyperref for figure
\newcommand{\tabref}[1]{\hyperref[#1]{Table \ref{#1}}} %Hyperref for table
\newcommand{\chapref}[1]{\hyperref[#1]{Chapter \ref{#1}}} %Hyperref for chapter
\renewcommand{\secref}[1]{\hyperref[#1]{Section \ref{#1}}} %Hyperref for section
\newcommand{\subsecref}[1]{\hyperref[#1]{Subsection \ref{#1}}} %Hyperref for subsection
\newcommand{\lstref}[1]{\hyperref[#1]{List \ref{#1}}} %Hyperref for list
\renewcommand{\appref}[1]{\hyperref[#1]{Appendix \ref{#1}}} %Hyperref for appendix
\renewcommand{\eqref}[1]{\hyperref[#1]{Equation (\ref{#1})}} %Hyperref for equation
\newcommand{\formularef}[1]{\hyperref[#1]{Formula (\ref{#1})}} %Hyperref for equation

\titleformat{\chapter}{\Huge\bfseries}{\thechapter .}{1em}{}
\titleformat{\section}{\LARGE\bfseries}{\thesection}{1em}{}
\titleformat{\subsection}{\Large\bfseries}{\thesubsection}{1em}{}
\titleformat{\subsubsection}{\normalsize\bfseries}{\thesubsubsection}{1em}{}

